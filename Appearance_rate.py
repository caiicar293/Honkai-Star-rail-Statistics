...
class HonkaiStatistics:
    def __init__(self, version, floor, node=0, by_ed=6 , by_cycle =30,by_ed_inclusive=False,by_ed_inclusive_combined=False, by_char = None,by_cycles_combined = 30,not_char=False,sustain_condition=None,star_num=None):
        self.version = version
        url = f"https://huggingface.co/datasets/LvlUrArti/MocData/resolve/main/{version}.csv"
        self.df = pd.read_csv(url)
        
        # self.version = version
        # self.csv_path = fr".\MocStats\data\raw_csvs\{self.version}.csv"
        # self.df = pd.read_csv(self.csv_path)

        # self.csv_path2 = fr".\MocStats\data\raw_csvs\{self.version}_char.csv"
        # self.df1 = pd.read_csv(self.csv_path2)
        
        with open(r'.\MocStats\data\characters.json' ) as file:
                info = json.load(file)
    ...
    def print_appearance_rate_by_char(self, by_avg_cycle=False, by_min_cycle=False, output = True,damage_dealers_only=False):
        ...
        with open(r'.\MocStats\data\characters.json' ) as file:
            ...
            rol = pd.DataFrame.from_dict(info, orient='index')
    ...
    def show_common_partners(self, char,output=True):
        honkai_stats =HonkaiStatistics(version=self.version, floor = self.floor,by_cycle=self.by_cycle, node=self.node, by_char=char,by_ed=self.by_ed)
    ...
    def apirori(self, sort_by='Samples', ascending=False, output=True, antecedent_filter=None, consequent_filter=None):
        ...
        t1 = time.time()
        data = HonkaiStatistics(version=self.version, floor=self.floor, node=self.node, by_cycle=self.by_cycle, by_ed=self.by_ed)
