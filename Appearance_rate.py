...
class HonkaiStatistics:
    def __init__(self, version, floor, node=0, by_ed=6, by_cycle=30, by_ed_inclusive=False, 
                 ...
                 not_char=False, sustain_condition=None, star_num=None, _method=False, 
                 _load_csv=pd.DataFrame(), _load_chars=pd.DataFrame()):
        
        self.version = version
        
        if _load_csv.empty and _load_chars.empty:
            folder = "raw_data"
            parquet_path = os.path.join(folder, f"{version}.parquet")
    ...
    def _process_data(self):
        # 1. PRE-COMPUTE LOOKUP TABLES (Massive Speedup)
        # Instead of calling .get().get() or set.intersection() inside the loop,
        # we calculate the truth values ONCE for all characters.
        ...
        # Pre-calculate if a character is a dps/specialist
        ...
        # 2. HANDLE NaNs FAST
        # math.isnan() inside a loop is slow. Fill empty eidolons with -1 beforehand.
        ...
        for row in self.df.itertuples(index=False):
            ...
            # --- TEAMS & ARCHETYPES ---
            if not self._method: 
                # Sort once using our fast pre-computed index
                n = tuple(sorted(chars, key=lambda d: char_index.get(d, 999)))
                
                # Build archetype using fast O(1) lookup
        ...
        if not self._method and self.node == 0:   
            # FIXED: Avoid zipping dictionary keys, access via UID mapping directly
            for uid, team_data in self.individual_teams.items():
                ...
                valid_combined = (self.by_ed_inclusive_combined is False and team_count == 2) or \
                                 (self.by_ed_inclusive_combined is True and max_eidolon == self.by_ed and team_count == 2)
                
                if valid_combined:
                    if team_data['Avg Cycles'] > self.by_cycles_combined:
                        continue
        ...
        if not self._method:
            for pair_keys, pair_data in self.combined_teams.items():
                pairs = chain(product(pair_keys[0], pair_keys[1]))
                for j in pairs:
    ...
    def print_appearance_rates(self,by_avg_cycle = False,by_min_cycle = False,by_char1 = None,by_char2 = None,by_char3 = None,by_char4 = None,least=None,
        ...
        # Collect characters to filter by
        m = []
        p = []
        if by_char1:
            if not not_by_char1:
        ...
        # Sort the dictionary based on the specified criteria
        if by_avg_cycle:
            sorted_teams = dict(sorted(self.teams.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_teams = dict(sorted(self.teams.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
        ...
        if output:
            ...
            print(f"Total Samples {self.total_samples} by team for MOC version {self.version}, Node {self.node}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
        ...
        for rank, (team, data) in enumerate(sorted_teams.items(), start=1):
            ...
            rows.append({
                ...
                'Median Cycles': round(np.median(data['Avg Cycles']), 2),
                '75th Percentile Cycles': round(np.percentile(data['Avg Cycles'], 75), 2),
                'Average Cycles': round(np.average(data['Avg Cycles']), 2),
                'Std Dev Cycles': round(np.std(data['Avg Cycles'], ddof=1), 2),  # Uses sample standard deviation
                'Max Cycles': max(data['Avg Cycles']),
            })
    ...
    def print_appearance_rates_both_sides(self,by_avg_cycle = False,by_min_cycle = False, 
        ...
        # Sort the dictionary based on the specified criteria
        if by_avg_cycle:
            sorted_teams = dict(sorted(self.combined_teams.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_teams = dict(sorted(self.combined_teams.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
        ...
        if output:
            # Print appearance rate and sample size for each team
            print(f"Total Samples {self.total_samples2} by team for MOC version {self.version}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
        for rank, (team, data) in enumerate(sorted_teams.items(), start=1):
            # Apply filtering if character filters are provided
            ...
            'Appearance Rate (%)': round(appearance_rate, 2),
            'Samples': data['Samples'],
            'Min Score': min(data['Avg Cycles']),
            '25th Percentile': round(np.percentile(data['Avg Cycles'], 25), 2),
            'Median Score': round(np.median(data['Avg Cycles']), 2),
            '75th Percentile': round(np.percentile(data['Avg Cycles'], 75), 2),
            'Average Score': round(np.average(data['Avg Cycles']), 2),
            'Std Dev': round(np.std(data['Avg Cycles'], ddof=1), 2),  # Uses sample standard deviation
            'Max Score': max(data['Avg Cycles']),
        })
    ...
    def print_archetypes(self,by_avg_cycle = False,by_min_cycle = False,by_char1 = None,by_char2 = None,by_char3 = None,by_char4 = None,least=None,
        ...
        # Sort the dictionary based on the specified criteria
        if by_avg_cycle:
            sorted_archetypes = dict(sorted(self.archetypes.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_archetypes = dict(sorted(self.archetypes.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
        ...
        if output:
            # Print appearance rate and sample size for each team
            print(f"Total Samples {self.total_samples} by archetype for MOC version {self.version}, Node {self.node}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
    ...
    def print_archetypes_both_sides(self,by_avg_cycle = False,by_min_cycle = False, 
                                          by_char1_n1 = None,by_char2_n1 = None,by_char3_n1 = None,by_char4_n1 = None,
                                          by_char1_n2 = None,by_char2_n2 = None,by_char3_n2 = None,by_char4_n2 = None,
        ...
        # Sort the dictionary based on the specified criteria
        if by_avg_cycle:
            sorted_archetypes = dict(sorted(self.combined_archetypes.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_archetypes = dict(sorted(self.combined_archetypes.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
        ...
        if output:
            # Print appearance rate and sample size for each team
            print(f"Total Samples {self.total_samples2} by archetype for MOC version {self.version}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
        for rank, (archetype, data) in enumerate(sorted_archetypes.items(), start=1):
            # Apply filtering if character filters are provided
    ...
    def print_appearance_rate_by_char(self, by_avg_cycle=False, by_min_cycle=False, output = True,damage_dealers_only=False):
        # Sorting the dictionary based on the specified key
        if by_avg_cycle:
            sorted_chars = dict(sorted(self.chars.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_chars = dict(sorted(self.chars.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
            sorted_chars = dict(sorted(self.chars.items(), key=lambda i: i[1]['Samples'], reverse=True))

        # Printing the header
        if output:
            print(f"Total Samples {self.total_samples} of characters for MOC version {self.version}, Node {self.node}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
        ...
        for char, char_data in sorted_chars.items():
            ...
            data.append({
                ...
                'Min Cycles': min(char_data['Avg Cycles']),
                '25th Percentile Cycles': round(np.percentile(char_data['Avg Cycles'], 25), 2),
                'Median Cycles': round(np.median(char_data['Avg Cycles']), 2),
                '75th Percentile Cycles': round(np.percentile(char_data['Avg Cycles'], 75), 2),
                'Average Cycles': round(np.average(char_data['Avg Cycles']), 2),
                'Std Dev Cycles': round(np.std(char_data['Avg Cycles'], ddof=1), 2),  # Uses sample standard deviation
                'Max Cycles': max(char_data['Avg Cycles']),
    ...
    def print_appearance_rate_by_char_both_sides(self, by_avg_cycle=False, by_min_cycle=False, output = True,damage_dealers_only=False,ch1_filter=None,ch2_filter=None):
        # Sorting the dictionary based on the specified key
        if by_avg_cycle:
            sorted_chars = dict(sorted(self.combined_chars.items(), key=lambda i: np.average(i[1]['Avg Cycles'])))
        elif by_min_cycle:
            sorted_chars = dict(sorted(self.combined_chars.items(), key=lambda i: min(i[1]['Avg Cycles'])))
        else:
            sorted_chars = dict(sorted(self.combined_chars.items(), key=lambda i: i[1]['Samples'], reverse=True))

        # Printing the header
        if output:
            print(f"Total Samples {self.total_samples2} of characters for MOC version {self.version}, Node {self.node}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
    ...
    def print_uids_by_cycles(self, cycle):
        # Print the list of UIDs by team
        hon2 = HonkaiStatistics(version = self.version, floor = self.floor,node = self.node,by_ed=self.by_ed,by_cycle=cycle,by_char=self.by_char,not_char=self.not_char)
        print(f"Cycle:{cycle}\nEidolon:{self.by_ed} \nNode:{self.node} \nFloor:{self.floor}")
    ...
    def print_uids_by_cycles_combined(self, score):
        # Print the list of UIDs by team
        hon2 = HonkaiStatistics(version = self.version, floor = self.floor,node = self.node,by_ed=self.by_ed,by_cycles_combined=score,by_char=self.by_char)
        print(f"Score:{score}\nEidolon:{self.by_ed} \nNode:{self.node} \nFloor:{self.floor}")
    ...
    def plot_statistics(self, team_key):
        ...
        if team_key in self.teams:
            cycles = self.teams[team_key]['Avg Cycles']
            ...
            if cycles:
                ...
                print(f"Cycle Counts for Team {team_key}:")
                total = 0
                print(f"Sample Size: {sample_size}")
                
                for cycle_value in sorted(cycle_counts):  # Sort by cycle value
    ...
    def plot_statistics_char(self, char):
        ...
        if char in self.chars:
            cycles = self.chars[char]['Avg Cycles']
            ...
            if cycles:
                ...
                # Add titles and labels for histogram
                plt.title(f"Cycle Frequency for Character {char} for version {self.version}, Node {self.node} , up to {self.by_ed} Eidolon")
                plt.xlabel('Cycles')
                plt.ylabel('Frequency')
                plt.legend()
    ...
    def plot_statistics_all(self,cumulative =False,output=True):
        ...
        if cycles:
            ...
            for cycle_value in sorted(cycle_counts):
                count = cycle_counts[cycle_value]
                total += count
                percentile = (1 - (total / sample_size)) * 100
    ...
    def show_common_partners(self, char,output=True):
        honkai_stats =HonkaiStatistics(version=self.version, floor = self.floor,by_cycle=self.by_cycle, node=self.node, by_char=char,by_ed=self.by_ed,_method=True,_load_csv=self.df,_load_chars=self.rol)
