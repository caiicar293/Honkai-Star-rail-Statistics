...
class HonkaiStatistics_V2_APOC:
    ...
    def _process_data(self,lf,char_cols,cons_cols,dps_names,char_to_index):
        ...
        totals = (
            ...
            .agg([
                ...
                pl.col("round_num").alias("Total_Scores"), # Kept as list
        ...
        per_eidolon = (
            ...
            .agg([
                ...
                pl.col("round_num").alias("Scores"), # Kept as list
        ...
        # 4. Pivot
        # aggregate_function="first" preserves the Scores lists
        pivoted = per_eidolon.pivot(
            on="Eidolon_Level",
            index="Character",
            values=["Samples", "Scores", "Sustains"],
        ...
        header_cols = ["Character", "Total_Samples", "Total_Scores", "Total_Sustains","uids"]
    ...
    def get_eidolon_performance_df(self):
        ...
        # These usually look like "Scores_Eidolon 0", "Sustains_Eidolon 0", etc.
        cycle_cols = [c for c in self.char_stats.columns if "Scores_Eidolon" in c]
        ...
        # 2. Map Scores to Averages and Sustains to Percentages
        # We use the actual column names to ensure we don't miss anything
        for col in cycle_cols:
            label = col.replace("Scores_", "") # Result: "Eidolon 0"
            stat_exprs.append(
                pl.col(col).list.mean().round(2).alias(f"{label} Avg Scores")
            )
        ...
        # This finds every "Eidolon X Avg Scores" and "Eidolon X Sustain %" column
        new_stat_cols = sorted([
            c for c in df.columns 
            if "Avg Scores" in c or "Sustain %" in c
        ])
