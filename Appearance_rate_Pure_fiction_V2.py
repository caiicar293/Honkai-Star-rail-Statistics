...
class HonkaiStatistics_V2_Pure:
    def __init__(self, version, floor, node=0, by_ed=6, by_points =0, by_ed_inclusive=False,
        ...
        folder = "raw_data"
        path = os.path.join(folder, f"{version}_pf.parquet")
       ...
       # 5. AGGREGATE CHARACTERS (The "Unpivot" trick)
        ...
        def get_performance_stats(df, group_keys):
            ...
            def rollup_gear(df, gear_col, alias):
                # Step A: Aggregate Points per specific gear item
                return (
        ...
        header_cols = [
            "Character", "Total_Samples", "Total_Points", "Total_Sustains", 
    ...
    def _process_combined_data(self, combined,char_cols,cons_cols,dps_names,char_to_index):
        ...
        self.combined_team_stats = (
            ...
            .agg([
                pl.count("uid").alias("Samples"),
                pl.col("total_points").alias("Points"), # Keep as list for percentile calculations
                pl.col("uid").unique().alias("uids")
            ])
            .collect()
        )
        ...
        self.combined_char_stats = (
            combined.select(["uid", "total_points", "n1_chars", "n2_chars"])
            ...
            .agg([
                pl.count("uid").alias("Samples"),
                pl.col("total_points").alias("Points") # Changed to alias "Points" and keep as list for percentile calculations
            ])
            .collect()
        )
    ...
    def _plot_cycle_distribution(self, df: pl.DataFrame, round_num: str, eidolon_col: str, cumulative: bool, output: bool, title: str):
          ...
          # Drop rows with missing Points just in case
          ...
          # 2. Group by Points and Count Eidolons
          ...
          final_df = stats_df.with_columns(e_exprs).select(
              [pl.col(round_num).alias("Points"), "Count", "Percentile (%)",
          ...
          if output:
              ...
              # Extract the raw Points column to a standard python list for Matplotlib
              Points_list = df.get_column(round_num).to_list()

              plt.figure(figsize=(12, 6))
              plt.hist(Points_list, bins='auto', alpha=0.5, color='blue', edgecolor='black', label='Points Frequency')
