...
class HonkaiStatistics_V2_Pure_fiction_Batch:
        ...
        if self.node == 0 or self.node == "all":
            ...
            if self._has_starward_col and self.is_starward:
                ...
                SELECT
                    ...
                    n2.*,
                    n3.*,
                    (n1.n1_Points + n2.n2_Points + n3.n3_Points) AS total_Points,
                ...
                SELECT
                    n1.*,
                    n2.*,
                    (n1.n1_Points + n2.n2_Points) AS total_Points,
    ...
    def _process_data(self, lf, char_lf, char_cols, cons_cols, dps_names, char_to_index):
            ...
            if struct_cols:
                for col in struct_cols:
                    processed_col = (
                        ...
                        .agg([
                            pl.col("count").sum(),
                            pl.col("Points").explode() # Flattens lists of scores
                        ])
                        .group_by(index_cols)
                        .agg(
                            **{col: pl.struct(["name", "count", "Points"])}
                        )
                    )
        ...
        header_cols = [
            ...
            "Total_Samples", "Total_Points", "Total_Sustains", "Total_Full_Clears",
    ...
    def get_eidolon_performance_df(self):
        ...
        df = df.sort(["version", 'at_eidolon_level', "up_to_eidolon_level", "node", "Total_Samples"], descending=[True, False, False, True, True]).with_row_index("Rank", offset=1)

        # 3. Fixed: Changed search from "Samples_Eidolon" to "Samples" since the format changed
        new_stat_cols = sorted([
            c for c in df.columns 
            if ("Samples" in c and "_" not in c) or "Avg Points" in c or "Sustain %" in c or "Full_Clear %" in c
        ])
