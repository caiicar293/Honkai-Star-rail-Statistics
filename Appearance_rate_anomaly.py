...
class HonkaiStatistics_Anomaly:
    ...
    def _process_data(self):
        exempts = ["Kafka", "Jing Yuan", "Seele", "Jingliu", "Dan Heng • Imbibitor Lunae",
                   ...
                   "Firefly", "Yunli", "Jiaoqiu", "Feixiao", "Lingsha","Rappa","Sunday","Fugue","The Herta","Aglaea","Tribbie","Mydei","Castorice","Anaxa","Hyacine","Cipher","Phainon","Archer","Saber","Hysilens","Cerydra","Evernight","Dan Heng • Permansor Terrae","Cyrene","The Dahlia","Yao Guang","Sparxie"]
    ...
    def print_appearance_rates(self,by_avg_cycle = False,by_min_cycle = False,by_char1 = None,by_char2 = None,by_char3 = None,by_char4 = None,least=None,
        ...
        sustains= [ "Fu Xuan", "Luocha", "Huohuo", "Aventurine", "Lingsha", "Gallagher","Bailu", "Gepard", "Lynx","Natasha","Hyacine","Dan Heng • Permansor Terrae","Dan Heng • Permansor Terrae"]
    ...
    def print_appearance_rates_both_sides(self,by_avg_cycle = False,by_min_cycle = False, 
        ...
        if self.floor != 0:
            return "Floor needs to be set to 0"
        ...
        if output:
            # Print appearance rate and sample size for each team
            print(f"Total Samples {self.total_samples2} by team for Anomly version {self.version}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
        for rank, (team, data) in enumerate(sorted_teams.items(), start=1):
            # Apply filtering if character filters are provided
    ...
    def print_archetypes_both_sides(self,by_avg_cycle = False,by_min_cycle = False, 
        ...
        if output:
            # Print appearance rate and sample size for each team
            print(f"Total Samples {self.total_samples2} by archetype for Anomly version {self.version}, under/equal {self.by_cycle} cycles up to {self.by_ed} Eidolon, floor {self.floor}")
    ...
    def print_appearance_rate_by_char_both_sides(self, by_avg_cycle=False, by_min_cycle=False, output = True,damage_dealers_only=False,ch1_filter=None,ch2_filter=None):
        ...
        for char_pair, char_data in sorted_chars.items():
            ...
            if isinstance(char_pair, tuple) and len(char_pair) == 3:
                char1, char2,char3 = char_pair
            else:
                char1, char2,char3 = char_pair, None
        ...
        if damage_dealers_only:
             ...
             df = df[df['Character 3'].apply(lambda x: pd.notna(x) and x in rol.index and set(self.rol.loc[x, 'role']).intersection({"dps", "specialist"}))]
             df['Rank'] = range(1, len(df) + 1)
        if ch1_filter:
    ...
    def print_uids_by_cycles(self, cycle):
        # Print the list of UIDs by team
        hon2 = HonkaiStatistics_Anomaly(version = self.version, floor = self.floor,node = self.node,by_ed=self.by_ed,by_cycle=cycle,by_char=self.by_char,not_char=self.not_char)
        print(f"Cycle:{cycle}\nEidolon:{self.by_ed} \nNode:{self.node} \nFloor:{self.floor}")
    ...
    def print_uids_by_team_combined(self, team):
        ...
        new = self.df
        fil = new[(new['uid'].isin(self.combined_teams[team]['uids'])) ]
        pd.set_option('display.width', 1000)  # Adjust the total width of the output
        pd.set_option('display.max_columns', None)  # Ensure all columns are shown
    ...
    def print_uids_by_cycles_combined(self, score):
        # Print the list of UIDs by team
        hon2 = HonkaiStatistics_Anomaly(version = self.version, floor = self.floor,node = self.node,by_ed=self.by_ed,by_cycles_combined=score,by_char=self.by_char)
        print(f"Score:{score}\nEidolon:{self.by_ed} \nNode:{self.node} \nFloor:{self.floor}")
