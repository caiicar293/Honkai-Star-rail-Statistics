...
def get_weighted_stats_query(table_name, value_col, node_col=None):
    """Generates the CTE for weighted averages and weighted quantiles with specific ordering."""
    group_cols = "version, eidolon_level"
    if node_col:
        group_cols += f", {node_col}"
...
def process_mode(table_name, value_col, node_col, rank_order, output_file):
    print(f"Processing Standard Mode: {table_name}...")
    ...
    ORDER BY s.version DESC, s.{node_col} ASC, s.eidolon_level ASC
    """
    con.execute(f"COPY ({query}) TO '{output_file}' (FORMAT PARQUET);")
    con.sql(query).show(max_rows=5, max_width=10000)

def process_multi_mode(table_name, value_col, rank_order, output_file):
    print(f"Processing Multi-Mode (Dual/Triple): {table_name}...")
    ...
    ORDER BY s.version DESC, s.eidolon_level ASC
    """
    con.execute(f"COPY ({query}) TO '{output_file}' (FORMAT PARQUET);")
    con.sql(query).show(max_rows=5, max_width=10000)
...
# --- EXECUTION ---
# Standard Modes
process_mode("moc_distributions", "Cycles", "node", "DESC", "moc_output.parquet")
process_mode("pure_fiction_distributions", "Points", "node", "ASC", "pf_output.parquet")
process_mode("apoc_distributions", "Scores", "node", "ASC", "apoc_output.parquet")
process_mode("anomaly_distributions", "Cycles", "floor", "DESC", "anomaly_output.parquet")

# Multi-Modes
process_multi_mode("moc_stats_dual_distributions", "Cycles", "DESC", "moc_dual_output.parquet")
process_multi_mode("pure_fiction_stats_dual_distributions", "Points", "ASC", "pf_dual_output.parquet")
process_multi_mode("apoc_stats_dual_distributions", "Scores", "ASC", "apoc_dual_output.parquet")
process_multi_mode("anomaly_stats_triple_distributions", "Cycles", "DESC", "anomaly_triple_output.parquet")