"""
generate_teams_dashboard.py
----------------------------
Generates a self-contained team stats HTML dashboard for any supported game mode.

Supported modes:
    moc       — Memory of Chaos       (node 0/1/2)
    apoc      — Apocalyptic Shadow    (node 0/1/2)
    pf        — Pure Fiction          (node 0/1/2)
    anomaly   — Anomaly Arbitration   (floor 0/1/2/3/4)

Usage:
    python generate_teams_dashboard.py --mode moc     --version 4.2.3
    python generate_teams_dashboard.py --mode anomaly --version 4.2.3
    python generate_teams_dashboard.py --mode apoc    --version 4.2.3 --output ./Dashboards
    python generate_teams_dashboard.py --mode pf      --version 4.2.3 --db path/to/hsr.duckdb
...
TEMPLATE_NAME      = "teams_template.html.j2"
...
def fetch_rows(
    ...
    # Use double-quotes around "Sustain?" to handle the special character
    sql = f"""
        SELECT
            Rank,
            version,
    ...
    for raw in rel.fetchall():
        ...
        # Normalise the sustain column to a plain key the JS can read
...
def render_dashboard(
    ...
    out_file = output_dir / f"{cfg['mode_label'].lower().replace(' ', '_')}_{safe_version}_teams.html"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a team stats HTML dashboard from DuckDB.",
